        ...
        history.forEach((tx) => {            
            ...
            const html = `
                <tr style="font-size:20px">
                    <td><a class="link-info" href="https://tribaldex.blog/@${author}" target="_blank">@${author}</a></td>
                    <td><a class="link-info" href="https://tribaldex.blog/@${author}/${permlink}" target="_blank">${permlinkShort}...</a></td>
                    <td>${weightPercent}%</td>
    ...
    $("#swap").click(async function () {
        ...
        updateBurn(async function(canBurn, amount, currency, post_link) {
            ...
            if (canBurn) {
                ...
                // if does not have hive-engine tag
                const json_metadata = JSON.parse(post.json_metadata);
                console.log(json_metadata);
                if (!json_metadata.tags.includes("hive-engine") && !json_metadata.tags.includes("tribes")) {
                    $("#status").text("Post does not have hive-engine or tribes tag");
                    $("#loading").addClass("d-none");
                    $("#swap").removeAttr("disabled");
